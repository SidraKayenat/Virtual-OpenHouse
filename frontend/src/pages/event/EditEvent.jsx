  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        <label className="flex items-center gap-1.5 text-[12px] font-semibold tracking-wide"
          style={{ color: locked ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.5)", textTransform: "uppercase", letterSpacing: "0.07em" }}>
          {label}
          {required && !locked && <span style={{ color: "#f87171" }}>*</span>}
          {locked && (
            <span className="flex items-center gap-1 text-[9.5px] font-bold px-2 py-0.5 rounded-full normal-case tracking-normal"
              style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.25)", border: "1px solid rgba(255,255,255,0.08)" }}>
              <Lock size={9} /> Locked
            </span>
          )}
        </label>
        {hint && <span className="text-[11px]" style={{ color: "rgba(255,255,255,0.22)" }}>{hint}</span>}
      </div>
      {children}
      {error && (
    ...
    <div className="relative">
      {Icon && (
        <Icon size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none"
          style={{ color: locked ? "rgba(255,255,255,0.15)" : focused ? "#a78bfa" : "rgba(255,255,255,0.25)", transition: "color 0.18s" }} />
      )}
      <input
        {...props}
        disabled={locked}
        style={{
          ...
          borderColor: locked ? "rgba(255,255,255,0.06)" : focused ? "rgba(167,139,250,0.5)" : "rgba(255,255,255,0.1)",
        }}
    ...
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-3 gap-3">
        ...
        <button
          type="button"
          onClick={() => {
            onChange({ backgroundType: "upload", customBackground: value?.startsWith("http") ? value : "", customBackgroundPublicId: "" });
  ...
  return (
    <div className="flex flex-col gap-3">
      <div
        ...
        style={{ height: 180, background: preview ? "transparent" : "rgba(255,255,255,0.03)", border: preview ? "none" : "2px dashed rgba(255,255,255,0.1)" }}
      ...
      >
        ...
        {uploading ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-[11px]" style={{ color: "rgba(255,255,255,0.3)" }}>Uploading...</p>
          </div>
        ) : preview ? (
          <>
            ...
            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"
              ...
              <button onClick={(e) => { e.stopPropagation(); setPreview(""); onClear(); }}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-[12px]" style={{ color: "rgba(255,255,255,0.3)" }}>Click to upload {label}</p>
            <p className="text-[10px]" style={{ color: "rgba(255,255,255,0.2)" }}>JPG, PNG, WebP · Max 5MB</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="rounded-2xl overflow-hidden"
      ...
      <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <h2 className="text-white font-bold text-[14.5px]" style={{ fontFamily: "'Syne',sans-serif" }}>{title}</h2>
      </div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div
        ...
        style={{ background: "#1a1728", border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 32px 64px rgba(0,0,0,0.6)" }}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-bold text-[15px]" style={{ fontFamily: "'Syne',sans-serif" }}>Save Changes?</h3>
            <p className="text-[12px]" style={{ color: "rgba(255,255,255,0.35)" }}>Review the fields you've edited</p>
          </div>
        </div>
        ...
        {changes.length > 0 && (
          <div className="rounded-xl p-4 flex flex-col gap-2.5"
            ...
            <p className="text-[10.5px] font-bold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.25)" }}>
              Modified fields
            </p>
            {changes.map(({ field, from, to }) => (
              <div key={field} className="flex flex-col gap-0.5">
                <p className="text-[12px] font-semibold capitalize" style={{ color: "rgba(255,255,255,0.55)" }}>{field}</p>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[11px] px-2 py-0.5 rounded line-through truncate max-w-[130px]"
                    style={{ background: "rgba(248,113,113,0.1)", color: "#f87171" }}>{String(from) || "—"}</span>
                  <ChevronRight size={11} style={{ color: "rgba(255,255,255,0.2)" }} />
                  <span className="text-[11px] px-2 py-0.5 rounded truncate max-w-[130px]"
                    style={{ background: "rgba(52,211,153,0.1)", color: "#34d399" }}>{String(to) || "—"}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 py-2.5 rounded-xl text-[13px] font-medium transition-all"
...
function getDiff(original, current) {
  ...
  return Object.keys(current)
    .filter((k) => !LOCKED.includes(k) && String(current[k]) !== String(original[k] ?? ""))
    .map((k) => ({ field: LABELS[k] || k, from: original[k] ?? "", to: current[k] }));
}
...
function LivePreview({ form }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden sticky top-6"
      ...
      <div className="relative h-36 overflow-hidden" style={{ background: "linear-gradient(135deg,#1e1b30,#2d1f5e)" }}>
      ...
      <div className="p-4 flex flex-col gap-2">
        <h3 className="text-white font-bold text-[14px] leading-snug line-clamp-1" style={{ fontFamily: "'Syne',sans-serif" }}>
          {form.name || <span style={{ color: "rgba(255,255,255,0.2)" }}>—</span>}
        </h3>
        {tags.length > 0 && (
        ...
        <div className="flex flex-col gap-1 text-[11px]" style={{ color: "rgba(255,255,255,0.38)" }}>
          {form.liveDate && (
          ...
          {form.numberOfStalls && (
            <span className="flex items-center gap-1.5"><Users size={10} /> {form.numberOfStalls} stalls</span>
          )}
          {form.customBackground && (
            <span>🌐 {skyboxLabel}</span>
          )}
        </div>
      </div>
    </div>
...
export default function EditEvent() {
    ...
    (async () => {
      try {
        ...
        const mapped = {
          ...
          endTime: e.endTime || "",
          backgroundType: e.backgroundType || "custom",
          customBackground: e.customBackground || "/Environments/RetroSpaceSkybox.webp",
  ...
  const handleSaveClick = () => {
    if (!form.thumbnailUrl) { setError("Thumbnail is required"); return; }
        ...
        <div className="flex-1 flex flex-col">
          ...
          <main className="flex-1 flex items-center justify-center">
            <div className="flex flex-col items-center gap-4">
              <div className="w-8 h-8 rounded-full border-2 border-violet-500 border-t-transparent animate-spin" />
              <p className="text-[13px]" style={{ color: "rgba(255,255,255,0.3)" }}>Loading event…</p>
            </div>
          </main>
        </div>
      </div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <main className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-[13px] mb-4" style={{ color: "rgba(255,255,255,0.35)" }}>{error}</p>
              <Link to="/user/events" className="text-violet-400 underline text-sm">Back to My Events</Link>
            </div>
          </main>
        </div>
      </div>
  ...
  return (
    <div className="h-screen flex" style={{ background: "#0c0c0f", fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
        ...
        <main className="flex-1 overflow-y-auto px-6 md:px-8 py-7">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-white text-xl font-bold" style={{ fontFamily: "'Syne',sans-serif" }}>Edit Event</h1>
                <p className="text-[12.5px] mt-0.5 truncate max-w-sm" style={{ color: "rgba(255,255,255,0.35)" }}>{form.name}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={handleSaveClick} disabled={!isDirty}
                ...
                style={isDirty
                  ? { background: "linear-gradient(135deg,#7c3aed,#6d28d9)", border: "1px solid rgba(167,139,250,0.25)", boxShadow: "0 4px 16px rgba(124,58,237,0.3)", cursor: "pointer" }
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06, duration: 0.3 }}
            ...
            <span><strong>Event Name</strong> and <strong>Live Date</strong> cannot be changed after creation.</span>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_220px] gap-6">
            <div className="flex flex-col gap-5">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08, duration: 0.3 }}>
                <Section title="Basic Information" icon={Info} accent="#a78bfa">
                  ...
                  <Field label="Description" required hint={`${form.description.length}/1000`}>
                    <StyledTextarea name="description" value={form.description} onChange={handle} rows={4} maxLength={1000} />
                  </Field>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12, duration: 0.3 }}>
                <Section title="Schedule" icon={Calendar} accent="#60a5fa">
                  <Field label="Live Date & Time" locked>
                    <StyledInput icon={Calendar} locked value={form.liveDate} readOnly />
                  </Field>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16, duration: 0.3 }}>
                <Section title="Stalls & Environment" icon={Building} accent="#34d399">
                  <Field label="Number of Stalls" hint="1 – 500">
                    <div className="flex items-center gap-3">
                      ...
                      <StyledInput icon={Users} type="number" name="numberOfStalls" value={form.numberOfStalls} onChange={handle} min={1} max={500} />
                      <button type="button"
                        onClick={() => set("numberOfStalls", Math.min(500, parseInt(form.numberOfStalls || 0) + 1))}
                        className="w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center font-bold text-lg"
                        style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.6)" }}>
                        +
                      </button>
                ...
                <Section title="Media" icon={Image} accent="#fb923c">
                  <Field label="Event Thumbnail" hint="Upload an image">
                    <ImageUploader
                      ...
                      onClear={() => { set("thumbnailUrl", ""); set("thumbnailPublicId", ""); }}
              ...
              {/* Tags */}
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.24, duration: 0.3 }}>
              ...
              {/* Bottom save bar */}
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.28, duration: 0.3 }}
                className="flex items-center justify-between p-4 rounded-2xl"
                style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)" }}>
                <div>
                  {isDirty
                    ? <p className="text-[12.5px]" style={{ color: "#fbbf24" }}>{diff.length} field{diff.length !== 1 ? "s" : ""} modified</p>
                    : <p className="text-[12.5px]" style={{ color: "rgba(255,255,255,0.3)" }}>No changes made yet</p>
                  }
                </div>
                <div className="flex gap-3">
                  <button type="button" onClick={() => navigate("/user/events")}
                    className="px-4 py-2.5 rounded-xl text-[13px] font-medium transition-all"
                    style={{ color: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)" }}>
                    Cancel
                  </button>
                  <button type="button" onClick={handleSaveClick} disabled={!isDirty}
                    className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-[13px] font-semibold text-white transition-all"
                    style={isDirty
                      ? { background: "linear-gradient(135deg,#7c3aed,#6d28d9)", border: "1px solid rgba(167,139,250,0.25)", boxShadow: "0 4px 16px rgba(124,58,237,0.3)", cursor: "pointer" }
                      : { background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.3)", cursor: "not-allowed" }}>
      ...
      <AnimatePresence>
        {showConfirm && (
          <ConfirmModal changes={diff} onConfirm={handleConfirm} onCancel={() => setShowConfirm(false)} loading={saving} />
        )}
      </AnimatePresence>
    </div>
  );
