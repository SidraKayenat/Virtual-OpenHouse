...
export const notifyEventSubmitted = async (eventId, eventName, eventAdminId) => {
  const title = "Event Submitted";
  const message = "Your event request has been submitted and is awaiting admin approval";
...
export const notifyEventRejected = async (eventId, eventName, eventAdminId, rejectionReason) => {
...
export const notifyEventPublished = async (eventId, eventName, eventAdminId) => {
...
export const notifyRegistrationReceived = async (eventId, eventName, eventAdminId, userName, registrationId) => {
...
export const notifyAdminPendingApproval = async (eventId, eventName, creatorName) => {
  try {
 ...
 */
export const createBulkNotifications = async (userIds, title, message, type, referenceId, referenceModel) => {
  try {
...
export const notifyRegistrationSubmitted = async (registrationId, eventName, participantId) => {
...
export const notifyRegistrationApproved = async (registrationId, eventName, participantId, stallNumber) => {
...
export const notifyRegistrationRejected = async (registrationId, eventName, participantId, rejectionReason) => {
...
export const notifyRegistrationCancelledAdmin = async (registrationId, eventName, eventAdminId, userName) => {
...
export const notifyStallCreated = async (stallId, eventName, stallNumber, ownerId) => {
...
export const notifyEventStartingSoon = async (eventId, eventName, participantId) => {
...
export const notifyWelcomeNewUser = async (userId, userName) => {
  ...
  const message = "Welcome to Virtual OpenHouse! Get started by browsing events.";
...
export const notifyAllStallOwnersEventStarting = async (eventId, eventName) => {
  try {
    ...
    const stalls = await Stall.find({ event: eventId })
      .select("owner")
      .lean();
    ...
    const notificationPromises = ownerIds.map(ownerId =>
      notifyEventStartingSoon(eventId, eventName, ownerId)
        .catch(err => {
          console.error(`Failed to send event starting soon notification to ${ownerId}:`, err);
          return null;
        })
    );
    ...
    console.log(`Event starting soon notifications sent: ${successCount}/${stalls.length}`);
    return {
      success: true,
      sent: successCount,
      total: stalls.length,
    ...
    console.error("Error notifying stall owners of event starting soon:", error);
    return {
      success: false,
      error: error.message,
    };
 ...
 */
export const notifyEventReminder24h = async (eventId, eventName, eventAdminId) => {
...
export const notifyAllParticipantsReminder24h = async (eventId, eventName) => {
  try {
    ...
    const ownerIds = stalls.map(stall => stall.owner);
    const notificationPromises = ownerIds.map(ownerId =>
      notifyEventReminder24h(eventId, eventName, ownerId)
        .catch(err => {
    ...
    console.log(`24-hour event reminders sent: ${successCount}/${stalls.length}`);
    return {
      success: true,
      sent: successCount,
      total: stalls.length,
 ...
 * For Event Admin + All Participants (Stall Owners)
 */
export const notifyEventReminder1h = async (eventId, eventName, eventAdminId) => {
...
export const notifyAllParticipantsReminder1h = async (eventId, eventName) => {
  try {
    ...
    const ownerIds = stalls.map(stall => stall.owner);
    const notificationPromises = ownerIds.map(ownerId =>
      notifyEventReminder1h(eventId, eventName, ownerId)
        .catch(err => {
    ...
    console.log(`1-hour event reminders sent: ${successCount}/${stalls.length}`);
    return {
      success: true,
      sent: successCount,
      total: stalls.length,
...
export const notifyAllParticipantsEventEnded = async (eventId, eventName) => {
  try {
    ...
    const ownerIds = stalls.map(stall => stall.owner);
    const notificationPromises = ownerIds.map(ownerId =>
      notifyEventEnded(eventId, eventName, ownerId)
        .catch(err => {
          console.error(`Failed to send event ended notification to ${ownerId}:`, err);
          return null;
        })
    );
    ...
    console.log(`Event ended notifications sent: ${successCount}/${stalls.length}`);
    return {
      success: true,
      sent: successCount,
      total: stalls.length,
...
export const notifyUsersWhoSetReminders = async (eventId, eventName) => {
  try {
    ...
    console.log(`24-hour reminders sent to ${successCount} users for event ${eventName}`);
    return {
      success: true,
      sent: successCount,
